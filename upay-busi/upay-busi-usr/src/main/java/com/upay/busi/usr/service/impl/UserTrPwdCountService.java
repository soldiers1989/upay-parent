...
import com.upay.busi.usr.service.dto.UserTrPwdCountDto;
...
/**
 * 用户交易密码错误次数统计： 1.临时锁定解锁判定 ； 2.判定输入密码是否正确，正确则清空相应数据并更改用户状态；
 ...
 * 
 */
public class UserTrPwdCountService extends AbstractDipperHandler<UserTrPwdCountDto> {
    @Resource
    IDaoService daoService;


    @Override
    public UserTrPwdCountDto execute(UserTrPwdCountDto userTrPwdCountDto, Message arg1) throws Exception {
        // TODO Auto-generated method stub
        Date sysTime = SysInfoContext.getSysTime(); // 系统时间
        String userId = userTrPwdCountDto.getUserId(); // 用户 ID
        String pwdCheckFlag = userTrPwdCountDto.getTraPwdCheckFlag();// 错误密码统计标识
                                                                     // Y需要统计
                                                                     // N不需要统计
        // 获取用户信息
        ...
        if (!DataBaseConstants_USR.PWD_ERROR_COUNT_N.equals(pwdCheckFlag)) {
            ...
            if (usrPwdListPo == null) {
                ...
                // 当日 交易密码连续错误次数，设置为1
                usrPwdListAddPo.setLogDayErr(0);
                // 交易密码总错误次数
                usrPwdListAddPo.setLogTotErr(0);
                // 当日交易密码连续错误次数，设为0
                usrPwdListAddPo.setTradeDayErr(1);
                // 交易密码总错误次数
                usrPwdListAddPo.setTradeTotErr(1);
                // 最后一次交易密码错日期
                usrPwdListAddPo.setLastTradepwdErrtime(sysTime);
                daoService.insert(usrPwdListAddPo);
            } else {
                // 如果不为空则更新记录
                int newTradeDayErr = 0;// 当日交易密码连续错误次数
                int newTradeTotErr = 0;// 交易密码连续累计错误总次数
                // 当日交易密码连续错误次数 + 1
                int tradeDayErr = usrPwdListPo.getTradeDayErr();
                newTradeDayErr = tradeDayErr + 1;
                // 获取交易密码错误总次数 + 1
                int tradeTotErr = usrPwdListPo.getTradeTotErr();
                newTradeTotErr = tradeTotErr + 1;
                // 查询系统参数表，获取到用户日交易密码连续错误次数锁定参数 3 次
                int tradeDayMaxErr =
                        new BigDecimal(ParmsContext.getParmByKey(CmparmConstants.TRADE_DAY_MAX_ERR)
                            .toString()).intValue();
                int tradeTotMaxErr =
                        new BigDecimal(ParmsContext.getParmByKey(CmparmConstants.TRADE_TOT_MAX_ERR)
                            .toString()).intValue();
                // 当每天连续交易密码错误次数小于3，并且每天总密码错误次数小于6 则 直接更新 密码错误统计表
                if ((newTradeDayErr < tradeDayMaxErr) && (newTradeTotErr < tradeTotMaxErr)) {
                    UsrPwdListPo usrPwdListUpPo = new UsrPwdListPo();
                    UsrPwdListPo usrPwdListWherePo = new UsrPwdListPo();
                    usrPwdListUpPo.setTradeDayErr(newTradeDayErr);
                    usrPwdListUpPo.setTradeTotErr(newTradeTotErr);
                    usrPwdListUpPo.setLastTradepwdErrtime(SysInfoContext.getSysTime());
                ...
                if ((newTradeDayErr == tradeDayMaxErr) && (newTradeTotErr < tradeTotMaxErr)) {
                    ...
                    userLockFlag =
                            userLockFlag.substring(0, 1).concat(DataBaseConstants_USR.PWD_LOCK_YES)
                    ...
                    // 获取锁定时效
                    int lockHour =
                            new BigDecimal(ParmsContext.getParmByKey(CmparmConstants.TRADE_TEMP_HOUR)
                                .toString()).intValue();
                    ...
                    usrPwdListUpdPo.setTradeTotErr(newTradeTotErr);
                    usrPwdListUpdPo.setLastTradepwdErrtime(SysInfoContext.getSysTime());
                    usrPwdListWherePo.setUserId(userId);
                    usrPwdListWherePo.setCountDate(SysInfoContext.getSysDate());
                ...
                if ((newTradeDayErr == tradeDayMaxErr) && (newTradeTotErr == tradeTotMaxErr)) {
                    ...
                    usrPwdlockBookUpPo.setLockFunction(DataBaseConstants_USR.LOCK_FUNCTION_TRADE_PWD_LOCK);
    ...
    private void expireUnlockTemporaryPwd(String userId, Date sysTime, UsrRegInfoPo usrRegInfo) { // 获取用户密码锁定登记簿相关信息
        ...
        if (StringUtils.isBlank(StringUtils.substring(usrRegInfo.getUserLockFlag(), 1, 2))) {
            ExInfo.throwDipperEx(AppCodeDict.BISPAY0001, "支付密码锁定标识");
        }
        UsrPwdlockBookPo usrPwdlockBookPo = new UsrPwdlockBookPo();
        // 状态为锁定
        usrPwdlockBookPo.setLockFlag(DataBaseConstants_USR.LOCK_FLAG_LOCK);
