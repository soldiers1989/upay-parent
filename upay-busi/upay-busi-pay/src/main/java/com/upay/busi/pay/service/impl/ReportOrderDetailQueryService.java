...
/**
 * Created hry 报表交易明细查询
 */
public class ReportOrderDetailQueryService extends
	...
	public ReportOrderDetailQueryDto execute(
		...
		if(tempstr.length()==16){tempstr=tempstr+0;}
		...
		List<Map<String, Object>> resultmap = daoService.selectList(
				PayOrderListPo.class.getName() + ".selectOrderDetail", map);
		...
		if (resultmap.size() > 0) {
			for (Map<String, Object> tempmap : resultmap) {
				...
				tempmaprp.put("orderNo", tempmap.get("ORDERNO").toString());
				tempmaprp.put("customerName", tempmap.get("CUSTOMERNAME")
						.toString());
				if(tempmap.get("ACCTNO")==null){
				tempmaprp.put("acctNO", "");
				}else{
				 tempmaprp.put("acctNO", tempmap.get("ACCTNO").toString());
				}
				tempmaprp.put("transAmt",
						String.format("%.2f ", tempmap.get("TRANSAMT")));
				String routeCode = null;
				if (tempmap.get("ROUTECODE") == null) {
					routeCode = "";
				} else {
					routeCode = tempmap.get("ROUTECODE").toString();
				}
				tempmaprp.put("routeCode", routeCode);
				tempmaprp.put("orderStat", tempmap.get("ORDERSTAT").toString());
				relist.add(tempmaprp);
			}
			reportOrderDetailQueryDto.setOrderDetailList(relist);
			Map<String, Object> countmap = daoService.selectOne(
					PayOrderListPo.class.getName() + ".countSelectOrderDetail",
					map);
	...
	private String buildReportForTxt(
		...
		try {
			...
			// 拼表格上面的内容
			bw.write("                               {dz48}云南红塔银行卷烟销售电子结算业务明细回单{dz24}");
			bw.write("\n");
			bw.write("\n");
			bw.write("\n");
			bw.write("  交易机构:"
			...
			if (detailList != null) {
				for (int i = 0; i < detailList.size(); i++) {
					...
					bw.write("│");
					bw.write(detailList.get(i).get("orderNo"));
					bw.write("│");
					bw.write(SignVerifyUtils.chineseStitchingSpace(detailList
							.get(i).get("customerName"), 10));
					bw.write("│");
					bw.write(String.format("%1$-20s",
							detailList.get(i).get("acctNO")));
					bw.write("│");
					bw.write(String.format("%1$-16s",
							detailList.get(i).get("transAmt")));
					bw.write("│");
					if (StringUtils.isBlank(detailList.get(i).get("routeCode"))) {
						bw.write(String.format("%-14s", ""));
					} else {
						bw.write(SignVerifyUtils.chineseStitchingSpace(
								detailList.get(i).get("routeCode"), 14));
					}
					bw.write("│");
					bw.write(SignVerifyUtils.chineseStitchingSpace(detailList
							.get(i).get("orderStat"), 14));
					bw.write("│");
					bw.write("\n");
					bw.write("  ├───────┼───────────┼─────┼──────────┼────────┼───────┼───────┤");
					// bw.write("├──────────────┼──────────────────────┼────────────┼────────────────────┼──────────────────────┼──────────────┼─────────────┤");
