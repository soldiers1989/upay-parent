 ...
 * 
 */
public class CheckWeixinCodeRepeatNotifyService extends AbstractDipperHandler<CheckUnionRepeatNotifyDto> {
    ...
    public CheckUnionRepeatNotifyDto execute(CheckUnionRepeatNotifyDto dto, Message message) throws Exception {
    	...
    	if(DataBaseConstants_PAY.ROUTE_CODE_CNAPSBEPS.equals(routeCode)){
    		//银联返回的平台流水号
    		String orderId = dto.getTransSubSeq();
    		if(StringUtils.isBlank(orderId)){
                ExInfo.throwDipperEx(AppCodeDict.BISPAY0001, "银联返回流水号");
            }
    		payFlowList.setTransSubSeq(orderId);
...
//    		}
    	...
    	dto.setAttach(dto.getAttach().split("##")[0]);
    	return dto;
    }
