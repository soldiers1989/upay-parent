 ...
 */
public class InitStlPaymentService extends AbstractDipperHandler<InitRouteSinglePaymentDto> {

	private static final Logger LOGGER = LoggerFactory.getLogger(InitStlPaymentService.class);
	@Resource
	...
	public InitRouteSinglePaymentDto execute(InitRouteSinglePaymentDto dto,
		...
		//代付是从商户结算账户里扣钱，所以付款方一定是商户的结算账户
		String stlAcctNo="";
		String stlAcctName="";
		String stlAcctType="";
		String merNo = dto.getMerNo();
		if(StringUtils.isNotBlank(merNo)){//当转账   付款方为商户时付款方为结算账户
			...
			stlAcctNo=dto.getStlAcctNo();
			stlAcctName=dto.getStlAcctName();
			stlAcctType=dto.getStlAcctType();
		}
