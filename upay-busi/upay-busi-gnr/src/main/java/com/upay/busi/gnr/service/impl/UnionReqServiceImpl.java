...
public class UnionReqServiceImpl extends AbstractDipperHandler<GeneralNoticeReqDto> {
    private static final Logger LOG = LoggerFactory.getLogger(UnionReqServiceImpl.class);
    ...
    public GeneralNoticeReqDto execute(GeneralNoticeReqDto dto, Message msg)
            throws Exception {
        LOG.info("银联通知类请求处理开始");

        String returnCode = dto.getRespCode();

        if ("00".equals(returnCode) || "A6".equals(returnCode)) {
     ...
     * @param dto
     */
    private GeneralNoticeReqDto updateFlowListByTransSubSeq(GeneralNoticeReqDto dto) {
