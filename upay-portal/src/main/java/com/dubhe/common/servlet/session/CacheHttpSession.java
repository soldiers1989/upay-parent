package com.dubhe.common.servlet.session;
...
import com.dubhe.common.util.SerializeUtil;
 ...
 * getSessionContext();
 * <p/>
 * 现在底层将Session的信息和储存在Session中的信息分别进行储存。只有当Session中的键值 对属性被改变时才会进行缓存的同步。
 *
 * @author weizhao.dong
 * @since 1.5
 */
public class CacheHttpSession implements HttpSession {
	 ...
	 * 会话在缓存中的KEY前辍
	 */
	public static final String SESSION_CACHE_KEY = "com.dubhe.session.";
	private static final LogUtil LOGGER = new LogUtil(
			LoggerFactory.getLogger(CacheHttpSession.class));
	private String id;
	 ...
	 *            失效时需要清理的Cookie名称 。
	 */
	public CacheHttpSession(ServletContext servletContext, String id) {
		this.id = id;
		cache = (IDipperCached) WebApplicationContextUtils
				.getWebApplicationContext(servletContext)
				.getBean("DIPPER_REDIS_CLIENT");
	 ...
	 */
	public void access() {
		sessionHeader.setLastAccessTime(Calendar.getInstance()
				.getTimeInMillis());
	}
	...
	public void setAttribute(String attributeName, Object attributeValue) {
		...
		Object oldValue = findCacheSessionAttribute().getAttribute(
				attributeName);
	...
	public void removeAttribute(String attributeName) {
		checkSessionInvalild();
		Object value = findCacheSessionAttribute().removeAttribute(
				attributeName);
		update = true;
	 ...
	 *            HttpSessionAttributeListener监听器列表.
	 */
	public void setSessionAttributeListeners(
		...
		if (sessionAttributeListeners != null) {
			this.sessionAttributeListeners = Arrays
					.copyOf(sessionAttributeListeners,
							sessionAttributeListeners.length);
		}
	}
	...
	public String toString() {
		...
		return G4Utils.replaceArgs(message, getId(), sessionCacheKeyHeader);
	}
	...
	/**
	 * 初始化方法。 初始化日志记录器。 初始化缓存中的属性容器。 如果缓存中没有相就内容即新建并设定创建时间和最后访问时间为当前时间和为新的会话。
	 */
	public void init() {
		if (!cache.keyExists(sessionCacheKeyHeader)) {
	...
	private void initCacheSessionHeader(boolean newBuild) {
		...
		this.updateCacheSessionHeader(sessionHeader);
		sessionAttribute = new CacheSessionAttribute();
		this.updateCacheSessionAttribute(sessionAttribute);
	 ...
	 *            true绑定还是false取消绑定.
	 */
	private void doHttpSessionBindingListener(String name, Object value,
			AccessType type) {
	 ...
	 * @param type
	 */
	private void doHttpSessionAttributeListener(String name, Object value,
			AccessType type) {
		HttpSessionAttributeListener listener;
		HttpSessionBindingEvent event = new HttpSessionBindingEvent(this, name,
				value);

		LOGGER.debugLog("cache Http sessionAttributeListeners: {0}",
				Arrays.toString(sessionAttributeListeners));
	...
	private void doHttpSessionListener(AccessType type) {
		...
		LOGGER.debugLog("cache Http Session listeners: {0}",
				Arrays.toString(sessionListeners));
	 ...
	 * @return 会话属性容器。
	 */
	private CacheSessionHeader findCacheSessionHeader()
			throws IllegalStateException {
		CacheSessionHeader header = null;
		try {
			header = (CacheSessionHeader) SerializeUtil.unserialize((byte[])cache.get(sessionCacheKeyHeader));
		} catch (Exception ex) {
	...
	private CacheSessionAttribute findCacheSessionAttribute() {
		...
		try {
			attribute = (CacheSessionAttribute)SerializeUtil.unserialize((byte[])cache.get(sessionCacheKeyAttribute));
			LOGGER.debugLog("环境中的消息是:cache.get={0}", attribute.toString());
		} catch (Exception ex) {
			// 应该找到的远程容器没有找到，所以重新构造一个。原有属性将丢失。
			LOGGER.warnLog("SessionCacheKey[{0}] is not found.",
	...
	private void updateCacheSessionHeader(CacheSessionHeader header) {
		cache.set(sessionCacheKeyHeader, SerializeUtil.serialize(header));
	}
	...
	private void updateCacheSessionAttribute(CacheSessionAttribute attribute) {
		cache.set(sessionCacheKeyAttribute, SerializeUtil.serialize(attribute));
	...
	private void removeRemoteSessionForCache() {
		cache.delete(sessionCacheKeyHeader);
		cache.delete(sessionCacheKeyAttribute);
	}
	...
	public static class CacheSessionHeader implements Externalizable {
		...
		public String toString() {
			...
			buff.append("CacheSessionHeader{").append("newbuild=")
					.append(newbuild);
	...
	public static class CacheSessionAttribute implements Externalizable {
		...
		public Serializable removeAttribute(String name) {
			...
			LOGGER.debugLog("From the Session {0} removed property.", name);
		...
		public int hashCode() {
			int hash = 3;
			hash = 61 * hash
					+ (this.attribute != null ? this.attribute.hashCode() : 0);
			return hash;
		}
		...
		public void readExternal(ObjectInput in) throws IOException,
			...
			if (seriaKeys != null && seriaValues != null
