...
public class DateUtil {
    ...
    public static boolean isQuarterEnd(Date nowDate) {
        ...
        cal.add(Calendar.DAY_OF_MONTH, 1);
        int day = cal.get(Calendar.DAY_OF_MONTH);
        if (day == 1 && (month == Calendar.MARCH || month == Calendar.JUNE || month == Calendar.SEPTEMBER
    ...
    public static boolean isQuarterBegin(Date nowDate) {
        ...
        int month = cal.get(Calendar.MONTH);
        int day = cal.get(Calendar.DAY_OF_MONTH);
        if (day == 1 && (month == Calendar.JANUARY || month == Calendar.APRIL || month == Calendar.JULY
    ...
    public static long betweenTimes(Date postDate, Date preDate) {
        ...
        if (subTime >= 0) {
            minute = MoneyUtil.divide(new BigDecimal(subTime), new BigDecimal(1000), null).longValue();
        }
        return minute;
    }
    ...
    public static Date getScopeDate(Date date, String dateScope) {
        ...
        if (CommunicationConstants.QUERY_SCOPE_WEEK.equals(dateScope)) {
            sysdateCal.add(Calendar.WEEK_OF_MONTH, -1);
        } else if (CommunicationConstants.QUERY_SCOPE_MONTH.equals(dateScope)) {
            sysdateCal.add(Calendar.MONTH, -1);
        } else if (CommunicationConstants.QUERY_SCOPE_THDMONTH.equals(dateScope)) {
            sysdateCal.add(Calendar.MONTH, -3);
        }else if(CommunicationConstants.QUERY_SCOPE_THDYEAR.equals(dateScope)){
            sysdateCal.add(Calendar.MONTH, -12);
        }
    ...
    public static Date getRealTime(Date date, String dateFlag) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        if (CommunicationConstants.PRE_TIME.equals(dateFlag)) {
     ...
     * @return
     */
    public static Date getPreDate(Date nowDate) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(nowDate);
    ...
    public static void main(String[] args) {
      String tempstr=format(new Date(),"yyyyMMddHHmmss");
      Date tempdate=parse(tempstr, "yyyyMMddHHmmss");
      System.out.println(tempstr);
      System.out.println(tempdate);
      System.out.println(format(addSecond(tempdate,86400),"yyyyMMddHHmmss"));
...
//        preCalendar.set(Calendar.DAY_OF_MONTH, 1);
        ...
        // long times = betweenTimes(postCalendar.getTime(),
        // preCalendar.getTime());
        // System.out.println(times/3600 + "小时" + (times/60 - times/3600 * 60) +
        // "分钟" + times%60 + "秒");

        // System.out.println(DateUtil.format(preCalendar.getTime(),
        // "yyyy-MM-dd HH:mm:ss"));
        // System.out.println(DateUtil.format(postCalendar.getTime(),
        // "yyyy-MM-dd HH:mm:ss"));
        // System.out.println(DateUtil.format(DateUtil.getRealTime(preCalendar.getTime(),
        // "1"), "yyyy-MM-dd HH:mm:ss"));
        // System.out.println(DateUtil.format(DateUtil.getRealTime(postCalendar.getTime(),
        // "2"), "yyyy-MM-dd HH:mm:ss"));

        /*System.out
            .println(DateUtil.format(getRealTime(preCalendar.getTime(), "1"), "yyyy-MM-dd HH:mm:ss:SSS"));
        System.out
            .println(DateUtil.format(getRealTime(preCalendar.getTime(), "2"), "yyyy-MM-dd HH:mm:ss:SSS"));*/
    }
}
