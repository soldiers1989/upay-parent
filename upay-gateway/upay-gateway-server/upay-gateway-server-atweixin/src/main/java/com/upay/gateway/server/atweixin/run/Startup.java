 ...
 * 
 */
package com.upay.gateway.server.atweixin.run;
...
public class Startup {
    public static void main(String[] args) {
        StartupUtil su = StartupUtil.getInstance("upay-gateway-server-atweixin", args);
        ...
        su.loadxml();
        URL url = Startup.class.getClassLoader().getResource("tsdk.properties");        
        File file = new File(url.getPath());
        SDKConfig.getConfig().loadPropertiesFromPath(file.getAbsoluteFile().getParent());
        Bootstrap.main(new String[] { "upay-gateway-server-atweixin" });
    }
}
