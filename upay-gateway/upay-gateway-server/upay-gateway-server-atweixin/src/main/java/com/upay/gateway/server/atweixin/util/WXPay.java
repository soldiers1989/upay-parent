...
public class WXPay {
	 ...
	 * @throws Exception
	 */
	public static Map<String, Object> fillRequestData(Map<String, Object> reqData,String tranCode) throws Exception {
		reqData.put("appid", SDKConfig.getConfig().getAppId());
		reqData.put("mch_id", SDKConfig.getConfig().getMchId());
		if (!tranCode.equals("SUBMCHMANAGEADD")&&!tranCode.equals("SUBMCHQRY")&&!tranCode.equals("ORDERQRYSINGLE")) {
			reqData.put("sub_appid", SDKConfig.getConfig().getSubAppid());
		}
	...
	public boolean isResponseSignatureValid(Map<String, Object> reqData) throws Exception {
		String signStr = (String) reqData.get(WXPayConstants.FIELD_SIGN);
	...
	public static Map<String, Object> post(Map<String, Object> reqData, String reqUrl, String encoding,String tranCode) {
        Map<String, Object> rspData =
                TransUtil.newParseValueBeforeTrans(tranCode, reqData, CmparmConstants.TRANS_TYPE_BEFORE);
		...
		String resultString = "";
		try {
			int status = hc.send(rspData, encoding);
			if (200 == status) {
				resultString = hc.getResult();
				if (null != resultString && !"".equals(resultString)) {
					// 将返回结果转换为map
				tmpRspData = WXPayUtil.newXmlToMap(resultString);
				}
			} else {
		 ...
		 return TransUtil.parseValueAfterTrans(tranCode, tmpRspData, CmparmConstants.TRANS_TYPE_AFTER);
	}
