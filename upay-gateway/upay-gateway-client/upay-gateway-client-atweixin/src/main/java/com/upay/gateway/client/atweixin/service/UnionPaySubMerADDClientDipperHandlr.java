 ...
 * @author hry
 */
public class UnionPaySubMerADDClientDipperHandlr extends AbstractAtWeiXinClientDipperHandler{
	@Override
	protected void setInitParam(Map<String, Object> paramMap) {
		paramMap.put("transCode", paramMap.get("tranCode"));
		paramMap.put("merchant_name",paramMap.get("merchantName"));// 商户名称  该名称是公司主体全称，绑定公众号时会对主体一致性校验
		paramMap.put("merchant_shortname", paramMap.get("merchantShortname")); // 商户简称  该名称是显示给消费者看的商户名称
		paramMap.put("service_phone",  paramMap.get("servicePhone")); // 客服电话  方便银联在必要时能联系上商家，会在支付详情展示给消费者
		paramMap.put("merchant_remark", paramMap.get("merchantRemark")); // 商户备注    同一个受理机构，特约商户“商户备注”唯一。商户备注重复时，生成商户识别码失败，并返回提示信息“商户备注已存在，请修改后重新提交”
		paramMap.put("channel_id", paramMap.get("channelId"));
		paramMap.remove("tranCode");
		paramMap.remove("merchantName");
		paramMap.remove("merchantShortname");
		paramMap.remove("servicePhone");
		paramMap.remove("merchantRemark");
		paramMap.remove("channelId");
		if(paramMap.containsKey("contactEmail")){
			paramMap.remove("contactEmail");
		}
		if(paramMap.containsKey("contactPhone")){
			paramMap.remove("contactPhone");
		}
	}
