...
public class UnionPayTranQueryClientDipperHandler extends
	...
	protected void setInitParam(Map<String, Object> init) {
		...
		init.put("accessType", "1");                            //接入类型，商户接入固定填0，不需修改
		init.put("acqInsCode", "05247410");                          //渠道类型
		init.put("merCatCode", "7011");                          //渠道类型
		//接入类型为收单机构接入时需上送
		init.put("merName", "银联收单接入测试商户");//C
		//接入类型为收单机构接入时需上送
		init.put("merAbbr", "测试商户");//C
		//</editor-fold>
