...
public class Alipay {
	...
	public Map subMchUpdate(Map<String, Object> targetBodyMap,
			Message message, String appId) throws AlipayApiException {
		log.info("子商户修改");
		byte[] alipayUpdateParam = loopMap(targetBodyMap);
		Map<String, Object> reqMap = (Map<String, Object>) SerializationUtils.deserialize(alipayUpdateParam);
		log.info("解析后参数" + reqMap);
		reqMap.putAll(targetBodyMap);
		AntMerchantExpandIndirectModifyRequest request = new AntMerchantExpandIndirectModifyRequest();
		request.setBizContent(JSONObject.toJSON(reqMap).toString());
		...
		Map map = AlipayUtil.parseJSON2Map(response.getBody());
		//支付宝新增商户时的参数
		byte[] alipayParam = SerializationUtils.serialize(reqMap);
		Map respMap = (Map) map.get("ant_merchant_expand_indirect_modify_response");
		respMap.put("alipay_param", alipayParam);
		return respMap;
	}
...
//		url = url.replace("http", "https");
		...
		log.debug("支付宝对账数据："+new String(ba));
