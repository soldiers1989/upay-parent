 ...
 * 
 */
public class UnionPayCollectionClientDipperHandler extends
	...
	protected void setInitParam(Map<String, Object> init) {
		init.put("tranCode", "12");
		init.put("txnSubType", "00");                           //交易子类型
		init.put("bizType", "000401");                          //业务类型
		...
		if(init.containsKey("certifId")){
			...
			if(certifId !=null){
				customerInfoMap.put("certifId",init.get("certifId").toString());
			}
		}
		...
		init.put("currencyCode", "156");                    	    //境内商户固定 156 人民币
