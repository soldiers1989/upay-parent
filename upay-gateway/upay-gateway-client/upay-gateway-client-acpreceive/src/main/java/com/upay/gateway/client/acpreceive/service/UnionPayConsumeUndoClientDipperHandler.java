 ...
 * 
 */
public class UnionPayConsumeUndoClientDipperHandler extends
	...
	protected void setInitParam(Map<String, Object> init) {
		init.put("tranCode", "31");
		init.put("txnSubType", "00");                           //交易子类型
		init.put("bizType", init.get("bizType"));                          //业务类型
		...
		init.put("txnAmt", init.get("txnAmt")); 
		
		init.put("currencyCode", "156");                  //交易币种(境内商户一般是156 人民币)
		init.put("origQryId", init.get("origQryId"));   			  //【原始交易流水号】，原消费交易返回的的queryId，可以从消费交易后台通知接口中或者交易状态查询接口中获取
