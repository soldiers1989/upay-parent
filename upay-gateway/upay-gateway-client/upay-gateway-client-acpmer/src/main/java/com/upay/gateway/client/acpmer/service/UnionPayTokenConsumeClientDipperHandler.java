 ...
 * 
 */
public class UnionPayTokenConsumeClientDipperHandler extends AbstractAcpClientDipperHandler{
	//默认配置的是UTF-8
	public static String encoding = "UTF-8";

	private Logger logger = LoggerFactory.getLogger(UnionPayTokenConsumeClientDipperHandler.class);
		@Override
		protected void setInitParam(Map<String, Object> init) {


			logger.info("无跳转支付 开始  ------begin  setInitParam");
			...
			init.put("orderId", init.get("transSubSeq")); // 商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
			...
			init.put("txnAmt", init.get("txnAmt").toString());							   //交易金额，单位分，不要带小数点
			...
			if(init.get("smsCode")!=null){
			Map<String,String> customerInfoMap = new HashMap<String,String>();
			if(init.containsKey("smsCode")){
				Object smsCode = init.get("smsCode");
				if(smsCode !=null){
					customerInfoMap.put("smsCode", smsCode.toString());			    	//短信验证码
				}
			}
		...
		protected void doErrorHandle(Map<String, Object> target, Fault fault) {
			...
			logger.info("无跳转支付  ------  doErrorHandle");
		...
		protected void doSuccessHandle(Map<String, Object> source,
			...
			logger.info("无跳转支付 结束  ------end");
		}

		@Override
		protected void doFailureHandle(Map<String, Object> source,
				Map<String, Object> target, Fault fault) {
			logger.info("无跳转支付  ------  doFailureHandle");
			target.putAll(source);
