...
public class TestRefundQuery {
	...
	public static void main(String[] args) throws Exception {
		...
		IDipperHandler<Message> handler = (IDipperHandler<Message>) ctx
				.getBean("AT_AlipayRefundQueryHandler");
		...
		map.put("outTradeNo", "201806101015123456123456"); // 商户订单号
		map.put("tradeNo", "");//支付宝交易号，和商户订单号不能同时为空	
		map.put("outRequestNo", "20180621010101051"); // 标识一次退款请求，同一笔交易多次退款需要保证唯一，如需部分退款，则此参数必传。
		m = handler.handle(m);
