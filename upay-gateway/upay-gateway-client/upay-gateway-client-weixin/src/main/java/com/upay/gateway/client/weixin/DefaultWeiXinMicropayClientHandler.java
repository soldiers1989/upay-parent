 ...
 * 
 */
public class DefaultWeiXinMicropayClientHandler extends AbstractClientDipperHandler {
    private static final Logger LOG = LoggerFactory.getLogger(DefaultWeiXinMicropayClientHandler.class);

    /** 微信方交易状态SUCCESS—支付成功 */
    public static final String WEIXIN_TRADE_SATTE_SUCCESS = "SUCCESS";
    /** REFUND—转入退款 */
    public static final String WEIXIN_TRADE_SATTE_REFUND = "REFUND";
    /** NOTPAY—未支付 */
    public static final String WEIXIN_TRADE_SATTE_NOTPAY = "NOTPAY";
    /** CLOSED—已关闭 */
    public static final String WEIXIN_TRADE_SATTE_CLOSED = "CLOSED";
    /** REVOKED—已撤销(刷卡支付) */
    public static final String WEIXIN_TRADE_SATTE_REVOKED = "REVOKED";
    /** USERPAYING--用户支付中 */
    public static final String WEIXIN_TRADE_SATTE_USERPAYING = "USERPAYING";
    /** PAYERROR--支付失败(其他原因，如银行返回失败) */
    public static final String WEIXIN_TRADE_SATTE_PAYERROR = "PAYERROR";
    ...
    protected void doSuccessHandle(Map<String, Object> source, Map<String, Object> target, Fault fault) {
        LOG.info("================远程访问成功，返回结果数据：" + String.valueOf(source.get("fileContext")));

        target.putAll(source);
       
        String bankType = (String) source.get("bankType");
        String tradeState = (String) source.get("tradeState");
        if (WEIXIN_TRADE_SATTE_SUCCESS.equals(tradeState)) {
            tradeState = T_PAY_ORDER_LIST_ORDER_STAT_Y;
        } else if (WEIXIN_TRADE_SATTE_REFUND.equals(tradeState)) {
            tradeState = T_PAY_ORDER_LIST_ORDER_STAT_REFING;
        } else if (WEIXIN_TRADE_SATTE_NOTPAY.equals(tradeState)) {
            tradeState = T_PAY_ORDER_LIST_ORDER_STAT_N;
        } else if (WEIXIN_TRADE_SATTE_CLOSED.equals(tradeState)
                || WEIXIN_TRADE_SATTE_REVOKED.equals(tradeState)) {
            tradeState = T_PAY_ORDER_LIST_ORDER_STAT_HC;
        } else if (WEIXIN_TRADE_SATTE_USERPAYING.equals(tradeState)) {
            tradeState = T_PAY_ORDER_LIST_ORDER_STAT_ING;
        } else if (WEIXIN_TRADE_SATTE_PAYERROR.equals(tradeState)) {
            tradeState = T_PAY_ORDER_LIST_ORDER_STAT_FAIL;
        }
        target.put("tradeState", tradeState);
    ...
    protected void doFailureHandle(Map<String, Object> source, Map<String, Object> target, Fault fault) {
        LOG.info("================远程访问失败，原因： 错误码：" + source.get("errCode") + ",,,,错误信息："
                + source.get("errMsg"));
        target.putAll(source);
    }
    ...
    protected void doErrorHandle(Map<String, Object> arg0, Fault fault) {
        ...
        LOG.info("================远程访问错误，原因： 错误码：" + fault.getOutCode() + ",,,,错误信息：" + fault.getOutMsg());
