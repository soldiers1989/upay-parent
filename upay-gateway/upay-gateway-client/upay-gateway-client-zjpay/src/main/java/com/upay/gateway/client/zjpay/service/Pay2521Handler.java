...
public class Pay2521Handler extends DefaultZJPayClientHandler {

    private static final Logger log = LoggerFactory.getLogger(Pay2521Handler.class);


    @Override
    protected void setInitParam(Map<String, Object> arg0) {
        log.info("快捷支付退款start");
        // head
        arg0.put(Channel._TRAN_CODE_NAME, "2521");// 交易码
        arg0.put("institutionID", CmparmConstants.GATEWAY_ZJPAY_INSTITUTION_ID); // 机构编号
        // body
        arg0.put("TxSN", arg0.get("txSN"));// 退款交易流水号
        arg0.put("PaymentNo", arg0.get("paymentNo")); // 原交易流水号（即2511接口中的paymentNo）
        arg0.put("Amount", arg0.get("amount"));// 退款金额 分 long
    ...
    protected void doErrorHandle(Map<String, Object> target, Fault fault) {
        ...
        switch (backCode) {
        case "30252106":
            fault.setMsgAll("支付交易流水号对应的订单不存在");
            break;
        case "2000":
