...
public class ChkByUnionPayStepToMer extends AbstractStepExecutor<Object, ChkThirdDetailPo> {
    private final static Logger logger = LoggerFactory.getLogger(ChkByUnionPayStepToMer.class);
    ...
    public void execute(BatchParams batchParams, int index, ChkThirdDetailPo data, Object object)
        ...
        // 开始进行数据勾兑:根据平台流水号、资金通道代码、平台订单号交易流水
        PayFlowListPo payFlowListPo = queryPmtFlowListInfo(chkThirdDetaiPo.getChnlSeq(),
                chkThirdDetaiPo.getOrgCode(), chkThirdDetaiPo.getOrderNo());
...
//        pmtFlowListPo.setRouteDate(chkDate);
     ...
     * @param chkDate 交易日期
     */
    public PayFlowListPo queryPmtFlowListInfo(String queryId, String routeCode, String orderId) {
        PayFlowListPo pmtFlowListPo = new PayFlowListPo();
        //pmtFlowListPo.setRouteDate(chkDate);
        pmtFlowListPo.setTransSubSeq(orderId);
        pmtFlowListPo.setRouteSeq(queryId);
        pmtFlowListPo.setRouteCode(routeCode);
        pmtFlowListPo = daoService.selectOne(pmtFlowListPo);
        return pmtFlowListPo;
    }
