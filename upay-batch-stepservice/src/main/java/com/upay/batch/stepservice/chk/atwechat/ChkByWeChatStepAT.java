...
public class ChkByWeChatStepAT extends AbstractStepExecutor<Object, ChkThirdDetailPo> {
    private final static Logger logger = LoggerFactory.getLogger(ChkByWeChatStepAT.class);
     ...
     * 
     * @Override public List<Object> getObjectList(BatchParams batchParams)
     *           throws BatchException { List list = new ArrayList();
     *           list.add(null); return list; // } }
     */

    /**
     * 获取微信交易明细与平台支付流水进行对账，对账以平台数据为准
     * 
     * @param batchParams
     * @param offset
     * @param pageSize
    ...
    public List<ChkThirdDetailPo> getDataList(BatchParams batchParams, int offset, int pageSize,
        ...
        logger.info("getTranDate:{}", DateUtil.format(batchParams.getTranDate(), pattern));
        logger.info("getPreDate:{}", DateUtil.format(batchParams.getPreDate(), pattern));
        logger.info("getBatchNo:{}",batchParams.getBatchNo());
        logger.info("getParameter:{}",batchParams.getParameter());
        String batchNo = batchParams.getBatchNo();
        logger.info("-----批次号：{}---获取第三方对账明细表---CPCN---(T_CHK_THIRD_DETAIL)数据流水数据开始--------", batchNo);
        // 获取第三方（微信）对账明细表(T_CHK_THIRD_DETAIL)数据
...
//        ChkThirdDetailPo chkThirdDetaiPo = (ChkThirdDetailPo) data;
        ...
        if (payFlowListPo == null) { // 微信多交易明细
            logger.info("微信多交易明细，微信交易流水：[{}]", chkThirdDetaiPo.getThirdSeq());
            // 记录差错信息
            ...
            if (payFlowListPo.getTransStat().equals(DataBaseConstants_PAY.T_PAY_TX_SUCCESS)
                    && chkThirdDetaiPo.getTransStat().equals(CommonConstants_GNR.OUT_PAY_STAT_SUCCESS)) { // 平台交易成功，微信交易成功

                if (chkThirdDetaiPo.getTransAmt().compareTo(payFlowListPo.getTransAmt()) != 0) {
                    // 金额不符
                    logger.info("平台支付流水[{}]交易状态与微信交易对账状态相符，金额不符", chkThirdDetaiPo.getChnlSeq());
                    // 记录差错信息
                    ...
                    logger.info("平台支付流水[{}]与微信对账成功", chkThirdDetaiPo.getChnlSeq());
                    // 更新第三方交易明细对账状态
                ...
                if (chkThirdDetaiPo.getTransStat().equals(CommonConstants_GNR.OUT_PAY_STAT_SUCCESS)) {
                    logger.info("平台支付流水[{}]交易失败，微信交易成功", chkThirdDetaiPo.getChnlSeq());
                    // 插入差错信息
                    insertChkErrListInfo(chkThirdDetaiPo, payFlowListPo,
                    ...
                    logger.info("平台支付流水[{}]交易成功，微信交易失败", chkThirdDetaiPo.getChnlSeq());
                    // 插入差错信息
                    insertChkErrListInfo(chkThirdDetaiPo, payFlowListPo,
...
//        pmtFlowListPo.setRouteCode(routeCode);
    ...
    public void insertChkErrListInfo(ChkThirdDetailPo chkThirdDetaiPo, PayFlowListPo payFlowListPo,
        ...
        chkErrListPo.setOrgCode(chkThirdDetaiPo.getOrgCode());// 微信资金通道代码
