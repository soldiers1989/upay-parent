...
import com.upay.batch.stepservice.migration.RemoveBatchAndGnrOperateLog;
...
public class RemoveBatchAndGnrOperateLogest {
	
	  private Logger logger = LoggerFactory.getLogger(RemoveBatchAndGnrOperateLogest.class);
	  
	    @Resource(name="RemoveBatchAndGnrOperateLog")
	    RemoveBatchAndGnrOperateLog removeBatchAndGnrOperateLog;
	    @Test
...
//		        batchParams.getParameter().put("endDate",endDate);
		       ...
		       batchParams.setTranDate(DateUtil.parse("2018-03-20", DataBaseConstans_ACC.DATE_FORMAT_YYYY_MM_DD));
//				List<PayOrderListHisPo> objectList = backupPayOrderList.getObjectList(batchParams);
		       removeBatchAndGnrOperateLog.execute(batchParams,0,null,null);
	    }
}
